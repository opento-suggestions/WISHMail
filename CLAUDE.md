...
the sixth signed run. **Nothing waits on a signature and nothing is armed**: no `--live` entry exists, the
counter is down, and the two homes the take spent are `--dry-run` behind a read-only allowlist.
**Twenty-four `check:*` are green** with `typecheck` and `p13:check`. The whole
