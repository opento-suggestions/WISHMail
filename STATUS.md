...
**Next — Phase B Step 2, and where its gates are.** Phase A is done and pushed: the 0.5.1 patch (D-135 – D-138), tagged `v0.5.1`. Phase B Step 1 is done: ADRs **D-139 – D-144** (§3.5's vacuity on a keyless doorbell; the three account roles; `$POSTAGE`'s parameters; the price topic's submit key; the first price list; where deployment entities are recorded), written before the first transaction. Writing them surfaced three defects, raised rather than patched as ledger §G items 9 – 11, and **Phase A′ — the 0.5.2 patch (D-145 – D-148)** — is the answer to them, also written before the first transaction: `validFrom` dropped from the `PriceList`, §4.6's `Conformance:` note naming the admin key, the provisioning template gaining the declaration registry topic, and `$POSTAGE` pre-minted 10,000 with the supply key on the treasury. Nothing has been submitted to `hedera:testnet`.

What remains stands up the Postmaster's operational entities, in this forced order, all of it `@hashgraph/sdk`:

1. Treasury account → Postmaster-agent account → **`$POSTAGE`** (treasury set at creation, `initialSupply: 0`, supply key the treasury's) → **mint 10,000 to the treasury as its own recorded step** (D-149) → associate the operator and the agent with it.
2. **The HIP-991 probe** (below) → price topic (`wishmail:prices:1`, submit and admin keys the operator's) → the first `PriceList` → doorbell, log, manifest topic per D-147's table.
3. Fill `spec/pins.json`'s `stampToken` for `hedera:testnet` — **and nothing else there** (§18.4; D-132's precedent). Everything else to `app/deployment/hedera-testnet.json`, with `app/OPERATIONS.md` carrying the method and the DIVERGENCE.

**Three reporting stops, each before signing anything further** — and the probe’s comes first, because it is the first write to consensus:

- **The Phase A′ gate — before any transaction.** The extract-and-diff empty both ways with the register at 83; `price-list.schema.json` validated positively against the first `PriceList` and negatively against the same message carrying `validFrom`; the `CHANGED` markers in the tree being exactly D-135's, D-136's, D-145's, D-146's and §18.2's combined marker; no wire string moved; and confirmation that D-148 touches no schema.
- **The HIP-991 probe, and its own gate report before any signature (D-149).** It is the first time this build writes to consensus, so it is reported before it is run: what it creates, what it asserts, what it reads back, and what it leaves on the network. The report is `app/OPERATIONS.md`, committed before anything is signed. The probe is **wholly disposable** — its own treasury, its own token, its own topic, its own stranger — and it touches neither the real `$POSTAGE` nor the real treasury nor the Postmaster-agent's topics. It **mirrors the real shape exactly**, because a probe that proves something about a different shape proves nothing about the template. What it settles: whether a fee collector must be associated with the denominating token (ledger §H holds this as MINE); the mirror node's actual name and shape for the fee-exempt list, which no recon here has ever seen returned; whether the exempt list is amendable under the admin key, which is `hip-991.md:110-113` observed rather than read; and which account `:101` debits when payer and signer differ, submitted **both ways** and recorded each time. **If testnet rejects the fee configuration, stop there. No HBAR substitute** (§4.4 admits no approximation). The probe writes nothing to `app/deployment/hedera-testnet.json` and nothing to `spec/pins.json`; `app/OPERATIONS.md` is its whole record. It leaves an inert token and its treasury on testnet permanently — `TokenDelete` needs an admin key the posture forbids — and that is stated in the report rather than discovered later.
- **The final Verification report:** the two-run idempotency result and the delete-one test against `agent.manifest`, every entity read back from the mirror node rather than from an SDK receipt, T-P7-4's two halves both positive, T-P17-1 under D-146, and the P-13 grep.

`HCS-10 by hand vs SDK` stays open and this phase does not decide it: it governs the operation grammar that carries envelopes, not the creation of a token or a topic. If creating a topic forces the decision, stop and report rather than deciding it implicitly.
