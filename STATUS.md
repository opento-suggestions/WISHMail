...
**Twenty-four `check:*` are green** with `typecheck` and `p13:check`, `check:letter` at 199 assertions,
`check:correspondent` at 149, `check:outputs` at 41 and `check:gzero2` at 45, and `check:captured` and `check:receipt` are byte-identical in their recorded digests
