...
1. Treasury account → Postmaster-agent account → **`$POSTAGE`** (treasury set at creation, 10,000 pre-minted, supply key the treasury's) → associate the operator and the agent with it.
...
**Three reporting stops, each before signing anything further:**
...
- **The HIP-991 probe.** Before the real doorbell is created, a scratch topic carrying a `CustomFixedFee` of one `$POSTAGE` collected by the treasury is created and read back from the mirror node, and the raw readback is reported: `denominating_token_id`, `collector_account_id`, and the fee-exempt list as the mirror node actually names and shapes it. It settles the one property HIP-991 does not state — whether a fee collector must be associated with the denominating token — which ledger §H still records as MINE. It also exercises the exempt list, amends it under the admin key to observe `:110-113` on testnet, and, because D-148 pre-mints, funds a scratch stranger and demonstrates the **charged** path positively rather than negatively. **If testnet rejects the configuration, stop there. No HBAR substitute** (§4.4 admits no approximation). The scratch topic is deleted under its admin key either way; the stranger is kept, since a token balance blocks `AccountDelete` and the T-P7-4 fixture can reuse it.
