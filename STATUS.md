...
**Where the build stands — 2026-09-08, end of Step 2.** Everything below is done and pushed to `main`.
...
**What is not done, and what it blocks.** `spec/pins.json` carries **thirty** unfilled pins — the twenty-eight `registeredSchemas` entries, which wait on HCS-13 registration, and the two `hedera:mainnet` fields, which wait on a network §15.5 leaves undeployed. **T-P9-2 refuses a report while any pin is unfilled, so no conformance claim is possible yet.** Standing up the entities did not make a claim possible; it made one eventually possible. `conformance/` holds no test file: none of the 83 exists. `app/` holds the provisioning scripts and nothing of the six-tool surface — no MCP server, no SDK, no CLI, no resolvers, no WebMCP page. The `hcs14` declaration — registry topic, HCS-11 profile file, account memo — is not built; D-147 put it downstream of the entities and its template row is complete and unexercised.
