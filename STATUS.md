...
**Next — Phase B Step 1, and where its gates are.** Phase A is done and pushed: the 0.5.1 patch (D-135 – D-138), tagged `v0.5.1`. Phase B stands up the Postmaster's operational entities on `hedera:testnet`, in this forced order, all of it `@hashgraph/sdk`:

1. ADRs **D-139 – D-144** (§3.5's vacuity on a keyless doorbell; the three account roles; `$POSTAGE`'s parameters; the price topic's submit key; the first price list; where deployment entities are recorded) — written **before** the first transaction.
2. Treasury account → Postmaster-agent account → **`$POSTAGE`** (treasury set at creation) → associate the operator and the agent with it.
3. **The HIP-991 probe** (below) → price topic (`wishmail:prices:1`, submit and admin keys the operator's) → the first `PriceList` → doorbell, log, manifest topic per D-138's table.
4. Fill `spec/pins.json`'s `stampToken` for `hedera:testnet` — **and nothing else there** (§18.4; D-132's precedent). Everything else to `app/deployment/hedera-testnet.json`, with `app/OPERATIONS.md` carrying the method and the DIVERGENCE.

**Two reporting stops, both before signing anything further:**

- **Step 2.5 — the HIP-991 probe.** Before the real doorbell is created, a scratch topic carrying a `CustomFixedFee` of one `$POSTAGE` collected by the treasury is created and read back from the mirror node, and the readback is reported: `denominating_token_id`, `collector_account_id`, `fee_exempt_key_list`. This settles the one property HIP-991 does not state — whether a fee collector must be associated with the denominating token — which ledger §H:329 still records as MINE. **If testnet rejects the configuration, stop there. No HBAR substitute** (§4.4 admits no approximation). The scratch topic is deleted under its admin key either way, and §H:329 goes from MINE to FETCHED.
- **The final Verification report**, per the plan: the two-run idempotency result, every entity read back from the mirror node, T-P7-4's two halves (the fee's collector and denomination; and that an owner's `connection_created` on its own doorbell assesses zero while a stranger's is charged one stamp), and the P-13 grep.
