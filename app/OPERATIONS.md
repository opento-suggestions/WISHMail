...
## THE RECORDED TAKE — the golden path on `gz5`, recorded. Gate report, written 2026-09-12 before any signature

**Written and committed before the first transaction, on the rule the probe, Step 2, Step 3, both Gate Zeros,
GATE FUND and Gate Four followed.** Gate Four’s report and run of record above are **not amended** by any of
this. Five markers, one prediction each, and **a divergence at any one is written down before the next is
attempted**.

### 0. What this run is, and what it is not

**It asks the implementation no new question.** Gate Four closed the last one four gates could not reach — a
brand-new HBAR-only wallet with zero association slots, carried through the whole lifecycle, with
`ensurePayerHoldsStamps`’ `TokenAssociateTransaction` branch executing against the network on both
operators. Nothing in the operator path is left unproved, and this report does not pretend otherwise.

**It exists to be recorded, and it is the first run of the golden path with the two nets in place.** That is
the one thing here that no gate has: **B3½**, a read-back of the payload *before* the irreversible step. It
exists because the model **re-encodes a string literal from its own reading of it rather than copying it** —
twice on consensus, at the second Gate Zero (69 bytes, *"…on consensu."*) and at Gate Four (38 bytes,
*"Certified agent mail,proven on Hedera."*, the same length as the literal and carrying padding the literal
lacks, which is the proof it was regenerated and not truncated). **Every prior run caught that after the
letter was on consensus, where a letter cannot be withdrawn. This is the first run that can catch it before a
stamp moves.**

**So a spoiled take is a rename and not a disaster.** `gz6-x`/`gz6-y` stand ready on
`0.0.10492960` and `0.0.10492962`, born under GATE FUND on the same terms and never provisioned.

### 0½. The HEAD, and why the ruling is satisfied in substance

**RECORD (Sonic): the take runs on the HEAD Gate Four ran on, `dc418dea29e9ac26f1bb423621f777eb59d0ef1c`.**
It runs at working-tree HEAD `e16dd71`, and **the two are the same binary**. Asserted rather than
claimed:

```
$ git diff --name-only dc418de..HEAD
  CLAUDE.md                                      STATUS.md
  ENTITIES.md                                    app/OPERATIONS.md
  docs/OPERATOR-SCRIPT.md                        conformance/reports/all.json
  app/deployment/demo-agents.hedera-testnet.json conformance/fixtures/gate-four-certified.json
```

**No `.ts`, no schema, no `package.json`, no registered fixture.** Six are documentation or the
record itself; `conformance/reports/all.json` differs only in its `generated` timestamp, the digest
either side being `51453eea…`; `demo-agents.hedera-testnet.json` is regenerated by
`entities:md`; and `gate-four-certified.json` is the capture, **deliberately absent from**
`RUN_FIXTURE_NAMES` so no body reaches it. The code that signs tonight is the code Gate Four ran.

**The battery at this HEAD, re-run and measured 2026-09-12:** `typecheck` clean, `p13:check` PASS,
**24 of 24 `check:*` PASS** (`check:letter` 199 assertions, `check:outputs` 41,
`check:gzero2` 45), conformance **87 registered · 44 passed · 43 failed**, report digest
`51453eea7d11dee8a5cfa71b84de826682d8283c543be48ec65c95625daeb6ec`.

### 1. The before-state, read from the mirror 2026-09-12

```
  gz5-x operator (SENDER)      0.0.10492954     250.00000000 ℎ   $POSTAGE  (not associated)   autoAssoc  0
  gz5-y operator (RECIPIENT)   0.0.10492957     250.00000000 ℎ   $POSTAGE  (not associated)   autoAssoc  0
  gz6-x operator (SPARE)       0.0.10492960     250.00000000 ℎ   $POSTAGE  (not associated)   autoAssoc  0
  gz6-y operator (SPARE)       0.0.10492962     250.00000000 ℎ   $POSTAGE  (not associated)   autoAssoc  0
  treasury                     0.0.10426205      20.00000000 ℎ   $POSTAGE       9915
  Postmaster payer             0.0.8641261     1868.27407048 ℎ   $POSTAGE          0

  DemoAgentX5  agent key 4a6310aa8769f5fe20acf345549bcd7b541473dd838341cac373804c2e8d342d   accounts under it: 0
  DemoAgentY5  agent key 7b3a7ff4dc8e44c502485c0e9ff881efb52dbeda8ff00994a46539e94861fe2e   accounts under it: 0

  $POSTAGE 0.0.10426208   supply 10000   decimals 0   supply key set
  PriceList 0.0.10426551  sequence 4 at 1789055861.123389104
      provisioning {"method":"hbar","registrationFee":"0.05","unitPrice":"30"}
      hbar method  12 $POSTAGE for "1.00" USD, rate from the network’s own exchangerate (D-170)
  exchange rate read 2026-09-12: 30000 hbar : 224053 cents  ->  1.00 USD = 13.38968905 ℎ
```

**Zero accounts under either agent key** is the precondition `buy_stamp` with `provision: true` refuses
on, and it is read here rather than assumed. **Both agent public keys above were also read back off the DRY
B0 cards** in the goose session store — the card’s `holder` is `s.agentPublicHex` and its
`buyer` is `s.homePayerId` (`app/sdk/server.ts:370-371`), both from the loaded home and
neither from the arguments, so the two cards prove which `config.json` and which `keystore.json` each
window holds.

### 2. What is already proved, and is therefore predicted rather than asked

**The association fires again on both operators.** Both read `0` slots and hold no `$POSTAGE`, so
`0 === -1` is false and `0 > 0` is false and the branch at `app/sdk/mailbox.ts` submits. Cost
**0.67094579 ℎ each — measured at Gate Four, twice**, against 0.62811176 and 0.62936798 at Step 4;
declared maximum 2 ℎ. It fires **after each agent’s purchase and before that agent’s first topic**, and it
fires on the **recipient’s** operator too, which never rings and ends associated with a balance of zero.
**Recorded, not fixed** (RECORD, Sonic): it is on the golden path and it is what Gate Four was called for.

This is a **repetition and not a new observable**, and the run of record will say so.

### 3. The five markers

**The ORDER is recipient before sender**, because her watcher must be running before he rings, and the labels
carry it rather than relying on X and Y.

---

#### **TAKE RECIPIENT** — `gz5-y`, DemoAgentY5, operator `0.0.10492957`

One `buy_stamp` call with `provision: true`, which is the whole of §4.6’s provisioned path (D-168).

**Predicted, in order:** a `TokenAssociateTransaction` for `0.0.10426208` on `0.0.10492957`,
paid and signed by that operator on its own client; **one atomic three-leg** `TransferTransaction` with the
Postmaster as payer — ℎ from `0.0.10492957` for the price, **12 `$POSTAGE`** from the treasury to
DemoAgentY5’s **public-key alias**, which is the leg that creates the account (HIP-542), and **0.05 ℎ** from
the Postmaster into that same alias for the agent to pay for its own registration; **five topics** — doorbell
(HIP-991 fee of one `$POSTAGE` to the treasury, the owner’s own key exempt), log, manifest, HCS-2
declaration registry, HCS-1 profile file — plus the profile’s chunks, a registry entry and an account memo, all
carried by the Postmaster; and a **StampReceipt** naming `txRef`, a `price`, `holder` = the
created account, and a `provisioning` block.

**Price predicted ≈43.39 ℎ** — 30 ℎ flat plus a 12-stamp bundle at "1.00" USD. **The stamp leg floats
with the network’s own rate at the moment of the purchase (D-170)**, so the exact figure is whatever the rate
is then: 13.38968905 ℎ at the rate read above, against 13.45267471 measured on 09-12 and 13.29198051 on
09-10. **Bound: under 50 ℎ, or the rate has moved by more than a third and that is itself the finding.**

**Write the created account id down — it is DemoAgentY5’s address, and B3 needs it.**

  **TAKE RECIPIENT — [ NOT YET ]**

---

#### **TAKE SENDER** — `gz5-x`, DemoAgentX5, operator `0.0.10492954`

The same call, the same shape, a second association and a second purchase. **Same prediction as TAKE
RECIPIENT**, with a different `holder`.
...
Provisioning already includes twelve stamps (P-2, D-168), so there is no separate purchase. The line exists so
that a top-up, if one is ever wanted, has a gate of its own rather than riding someone else’s.

  **TAKE STAMPS — [ N/A unless a top-up is wanted ]**

---

#### **TAKE SEND** — one certified letter, X5 → Y5, first contact and a return receipt in ONE call (§6.4)

**Predicted, in order:**

1. **§4.4’s hop fires.** The sender’s operator holds **zero `$POSTAGE`**, so `ringStamp` moves **one
   stamp from DemoAgentX5 to `0.0.10492954`** before the ring — the agent signs, the operator pays. Third
   time it has ever fired.
2. **The ring**: a `connection_request` on DemoAgentY5’s doorbell, whose HIP-991 fee **consumes that
   stamp** into the treasury.
3. **The lane is born on the RECIPIENT’s doorbell**, because she answers. Its submit key must be a **threshold
   of exactly the two agents’ keys and no other** (§7.1, T-P17-2) and it must carry **no custom fee** (T-P11-3).
4. **The manifest** to X5’s manifest topic; **the settlement**, one transfer of **two stamps** — one weight, one
   receipt fee — from DemoAgentX5 to the treasury under the memo `wishmail:<envelopeId>`, **naming that
   envelope and no other** (§4.3).
5. **Chunk 0** on the lane, **one chunk, 38 bytes** of payload.
6. A **ScheduleCreate** for the receipt, its inner body naming **the sender’s own side as payer** — so signing
   costs the recipient nothing (T-P16-2) — and a `transaction` operation announcing it on the lane.
7. `send` returns **chunk 0’s Postmark**, and returns **before anyone signs** (P-14, D-30). A Postmark is
   not a delivery.

**The payload is `Certified agent mail proven on Hedera.` — 38 bytes, 52 base64 characters, one `=` of
padding**, as `Q2VydGlmaWVkIGFnZW50IG1haWwgcHJvdmVuIG9uIEhlZGVyYS4=`. **The comma is dropped deliberately**
(RECORD, Sonic): punctuation with a space after it is what the model lost at Gate Four, so it is **cut rather
than risked**. That is a smaller target and **not a safe one** — the mechanism is re-encoding, not length, and
**B3½ is what actually closes it**.

**An `AttemptedDeliverySlip` is a RESULT and not a failure** (P-12): one stamp is gone at her door, no
postage was affixed, the request stands, and it would mean her watcher was not running. **Do not send again
without saying so** — a second ring is a second stamp and a second lane, and a lane cannot be closed.

  **TAKE SEND — [ NOT YET ]**

---

#### **TAKE ACK** — Y5 opens it and signs for it

**Predicted:** `inbox` returns one delivery, `opened: true`, **38 byte(s)**, carrying the **pending
schedule**; `payload` in `structuredContent` is a **base64 string** and not a Node `Buffer`; and
the card renders the sentence **labelled as a rendering of the bytes**. `ack` signs the schedule, **the
network executes it the instant the last signature lands**, and the receipt manifest is published to
**DemoAgentY5’s own manifest topic**. **Not one stamp of the recipient’s moves at any step (T-P16-2).**

  **TAKE ACK — [ NOT YET ]**

---

### 3½. THE TWO NETS, and they are the only thing here no gate had

**B3½ — the read-back, BEFORE the spend.** The model repeats the payload string, **calls no tool**, and the
operator compares 52 characters by eye before a stamp moves. It costs one turn. It is the **only** place the
payload can be checked, because `send` is irreversible. **A single character’s difference is a stop**:
correct it and have it read the string back again.

**B5 — the rendered-against-printed check, AFTER.** The card must render the sentence in full beside the
plaintext printed in B4. A missing character means the model re-encoded the base64 at B4, the letter is already
on consensus, and **the stop is: do not `ack`**. The take is spoiled and moves to the spare pair.

**B5 is only possible at all because `inbox` returns base64 rather than a Node `Buffer`** (C0.2, D-178,
2026-09-11), which is why a card a human can read is a safety property and not a courtesy. Gate Four’s
divergence was caught **in one glance** for exactly that reason.

### 4. The stamp arithmetic, predicted

```
                                  before        after      why
  DemoAgentX5  (sender)               0    ->      9       +12 bought, -1 the hop, -2 the settlement
  DemoAgentY5  (recipient)            0    ->     12       +12 bought, and CHARGED NOTHING (T-P16-2)
  0.0.10492954 (sender’s operator)    0    ->      0       +1 from the hop, -1 to the doorbell fee
  0.0.10492957 (recipient’s operator) 0    ->      0       associated, and never holds one
  treasury     0.0.10426205        9915    ->   9894       -24 sold, +1 doorbell fee, +2 settlement
```

A net **−21**, which is what the second Gate Zero and Gate Four each measured exactly on their own pairs.

**And in ℎ, predicted from Gate Four’s measured rows:** each operator ends near **205 ℎ** — 250 less
≈43.39 for the purchase, 0.671 for the association, and the post-purchase traffic Gate Four measured at
0.87604875 (sender: hop, ring, two outbound records, manifest, settlement, chunk 0, ScheduleCreate) and
1.23323344 (recipient: lane, `connection_created`, outbound record, ScheduleSign). **The Postmaster nets
about +27 ℎ** over the take, as it did at both prior gates. **GATE FUND’s four creations are funding and
are excluded**, being recorded in GATE FUND’s own table above; no line here is a sale.

### 5. GREEN, and it is unchanged

**Not `send` returning. Not any card. Not `ack` returning.** GREEN is two facts on a mirror node, read
afterward and by me and not by the operator:

1. the schedule’s **`executed_timestamp` is not null**, and
2. the **receipt manifest is on DemoAgentY5’s own manifest topic**, at a topic id and sequence number, hashing
   to what the schedule carried, **chained back to chunk 0’s postmark**.

Beside them, and not instead of them: a stranger holding no key, no account, no stamp and no home reads the
lane back **twice at one digest** (§11.7), and the letter appraises `unverified` with reason `T-P12-4`
— **correct output and not a shortfall**, because this release claims no profile (§1.5, §9.6, D-177).
...
- **A purchase that stops part way resumes by pasting the same instruction again.** It is idempotent **against
  consensus, not against local state**: `buy_stamp` with `provision` refuses if the holder’s account
  already exists, and `generateMailbox` resolves the agent’s own address first and does nothing if
  coordinates exist. It creates only what is missing and never buys twice.
- **If the counter’s record of a purchase is gone: STOP, and do not reconstruct the receipt.** A receipt is
  never rebuilt from the ledger by the party that charged (Correspondent A).
- **A lane cannot be closed.** Anything ambiguous about whether a ring happened is a stop, not a retry.

### 7. Every way it stops

- `--live` did not arrive → the card says `DRY RUN — … would …`. **In LIVE a "would do" card means the
  flag did not arrive. Stop.**
- the counter is down → `buy_stamp` cannot reach `127.0.0.1:4600`. **In LIVE that means the counter
  died**, not that the window is DRY; the heuristic inverts after the flip.
- **B0’s card names the wrong home** — `holder` reading `{"account": …}` instead of a public key, or
  `buyer` naming anything but `0.0.10492954` / `0.0.10492957`. Stop; run nothing below it.
- **B3½’s read-back differs by one character** → stop **before** B4. Nothing has been spent. Correct and repeat.
- `resolve` returns `RESOLVE_NOT_FOUND` after TAKE RECIPIENT → her provisioning did not finish. Stop.
- `send` returns an `AttemptedDeliverySlip` → her watcher was not running. A result, not a failure.
  Stop and say so.
- **B5’s rendering is short by even one character** → the model re-encoded the base64 at B4. The letter is on
  consensus and cannot be withdrawn. **Stop, do not `ack`.** The take moves to `gz6`, which costs a
  remove-and-rename in `config.yaml` and a second GATE-FUND-funded pair that already exists.
- `ack` returns `ACK_NOT_OPENED` → the schedule’s body names a different identifier, postmark or
  epoch. That is the check working. Stop.
- **On any stop: report what is true at the stop and what is resumable, and wait.** Never repair past it, never
  infer.

### 8. What this run will NOT prove, named in advance

- **No `hol` resolution.** `register_agent` is off the goose allowlist by design, as at every goose
  gate; T-P6-5 and T-P13-4 stay unexpanded.
- **No `verified` appraisal**, because this release claims no profile. `unverified` is correct output.
- **Nothing new about the brand-new wallet.** Gate Four closed it; this repeats it.
- **`payment` is accepted and ignored.** The published input schema requires it and validates it against a
  two-armed `oneOf`, and `app/sdk/server.ts` never reads `args['payment']`: the buyer is always the
  home’s own payer and the method is always `hbar`. **It fails safe** — no model-supplied value can
  redirect payment or choose a method — but it is the input side of the C0.2 class, found on the golden path
  during the 2026-09-12 dry pass, and it is **recorded here and not fixed**, the code being frozen.

### 9. The fill-in

**Nothing signs until this line says so, and it is Sonic’s to write.** AUTHORIZED means both goose entries
