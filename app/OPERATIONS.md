...
**Status: not yet run. Nothing has been signed.** This section is written before the first transaction this build submits to consensus, so that the record shows what was intended before it was done rather than after. Ruled 2026-09-08. It fills with observations when the probe runs; until then every line below is a statement of intent.

The probe exists to settle what HIP-991 at the pin does not state and the SDK does not document: whether a fee collector must be associated with the denominating token (ledger §H records this as MINE, "confirm when §14 lands", and §14 has landed), and whether a fee-exempt list amended under the admin key takes effect. It is **wholly disposable**: it creates its own treasury, its own token, its own topic and its own stranger, and the real `$POSTAGE`, the real treasury and the Postmaster-agent's own topics are not created, touched, or named by it. It mirrors the real shape exactly, because a probe that proves something about a different shape proves nothing about the template.

### 1. What it creates

| # | Entity | Shape | Warrant |
|---|---|---|---|
| 1 | probe treasury account | ED25519 key generated in-process, minimally funded by the operator | mirrors D-140's treasury role |
| 2 | probe token | `decimals: 0`, `initialSupply: 10000`, `supplyType: INFINITE`, supply key the probe treasury's, and **no admin, freeze, wipe, pause, KYC or fee-schedule key** | D-148's shape exactly |
| 3 | probe topic | **no submit key**; admin key held by the probe; a HIP-991 `CustomFixedFee` of **one unit** of the probe token collected by the **probe treasury**; **no fee schedule key**; a fee-exempt list carrying one throwaway key | D-138's doorbell row exactly |
| 4 | stranger account | ED25519 throwaway, minimally funded, associated with the probe token, holding one unit transferred from the probe treasury | the unexempted caller of §4.4 |
| 5 | two throwaway keys | one on the exempt list at creation, one to replace it | D-137's outcome, and `hip-991.md:110-113` |

The **operator pays every network fee** (D-47) — with one question under it, stated in §5 below rather than assumed away. Every private key here is born in the running process and none is written to `spec/pins.json`, to `app/deployment/hedera-testnet.json`, to a log line, or to a commit; the throwaway keys are not written anywhere at all, since nothing outlives the probe that would need them.

### 2. What it asserts

- **The token was built as D-148 says.** `initial_supply == 10000`, `total_supply == 10000`, `decimals == 0`, `supply_type == INFINITE`, `treasury_account_id` the probe treasury, `supply_key` the probe treasury's raw hex, and `admin_key`, `freeze_key`, `wipe_key`, `pause_key`, `kyc_key`, `fee_schedule_key` all six null.
...
- **The charged path.** The stranger's `connection_request`-shaped submission is assessed **one unit** of the probe token to the probe treasury — the T-P7-4 read, `assessed_custom_fees` on the transaction.
...
- `GET /api/v1/tokens/{probeToken}` — the twelve fields of §2.
...
- `GET /api/v1/transactions/{id}` for each submission — `result`, `assessed_custom_fees` with its amount, `token_id` and collector, the paying account, and `charged_tx_fee`. Also whether the response carries `entity_id`, which the provisioning journal would like to use and which ledger §H does not list.
...
- **The probe token cannot be deleted.** It carries no admin key, by D-148's shape, and `TokenDelete` requires one. Building it any other way would mean the probe did not mirror the template, which is the one thing it must do. It remains on `hedera:testnet` permanently, with 10,000 units.
...
- **The stranger account** can be emptied and dissociated and then deleted; whether it is, is a matter of tidiness rather than correctness, and the probe records what it did.

So the probe leaves an inert token and its treasury on testnet forever. That is the price of mirroring D-148's key posture honestly, it is paid on a test network, and it is written down here rather than discovered later by someone reading a mirror node and wondering what the second `$POSTAGE`-shaped token is. **Neither entity is written to `app/deployment/hedera-testnet.json` or to `spec/pins.json`** — those are the ledgers of what the deployment *keeps*, and the deployment keeps none of this. This file is the whole record of the probe.

### 5. The one thing not settled before signing

HIP-991 charges the fee to the account submitting the message (ledger §H, `hip-991.md:101`), and a Hedera transaction has one payer. Two readings of the probe's charged path follow, and they create different entities:

- **The stranger pays.** The stranger is the payer of its own submission, so it needs HBAR as well as the one unit, and the operator's D-47 role is to have funded it. This is the shape described above, and it tests the fee mechanism directly.
- **The operator pays**, which is what §4.4 actually describes for a doorbell: the sender transfers a stamp *to the Postmaster*, and the Postmaster pays the fee — which is why D-140 makes the operator the "momentary bearer of the first-contact stamp" and why the operator must be associated with `$POSTAGE` at all. On this reading the probe token must be funded to the **operator**, not to the stranger, and the stranger contributes only a signature that is absent from the exempt list — which is exactly how D-137's exemption works, since HIP-991 waives **by signature** even with the Postmaster as payer.

The second reading is the one the specification describes; the first is the one the probe as ruled creates. They can be reconciled by having the probe do both submissions and record who was debited each time, which costs one extra transaction and settles the routing question for §4.4 at the same time as the fee question. Raised here rather than resolved silently.
